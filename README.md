...
**Calendario emergente en waybar**: ícono nuevo (`custom/calendar`, antes del reloj) que hace toggle de [gsimplecal](https://github.com/dmedvinsky/gsimplecal) vía `chocomazapan-toggle-calendar`. `hypr/core/apps/calendar.lua` lo posiciona centrado y **superpuesto un poco por debajo de la barra** (`move = {"(monitor_w/2-window_w/2)", "16"}`, la barra mide 26px + 4px de margen arriba) — como la barra vive en una capa de Wayland más alta (`layer: top`), su borde redondeado tapa el borde redondeado de arriba del calendario, dando el efecto de protuberancia que sale de la barra en vez de un popup flotante separado con hueco.
