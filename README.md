...
- **Apps personales** — `install.sh` instala el escritorio completo (shell, barra, tema, y las apps a las que apuntan los binds/scripts: `nautilus`, `xdg-terminal-exec`, `satty`/`slurp`/`hyprpicker`/`tesseract` para capturas, `impala`/`bluetui`/`wiremix` para wifi/bt/audio) y, en un prompt aparte (`PKGS_APPS`, default sí), el set base de apps: navegador Zen, Obsidian, KeePassXC, mpv, imv, Evince, ZapZap, VS Code. Lo demás (Signal, Spotify, 1Password, ...) se instala a mano — `install.sh` lo lista al final.
...
**Apps GTK / libadwaita — Yaru** (`chocomazapan-gtk-theme-sync`, corre en `sync_desktop()`): el acento del wallpaper es un hex arbitrario, pero Yaru (el tema de Ubuntu) solo trae 9 variantes de color fijas — se elige la más cercana al `accent` de `colors.toml` **por tono (hue)**, no por distancia RGB: el acento extraído suele venir desaturado y la distancia RGB cruda lo mandaba a la variante equivocada (un periwinkle gris `#98a3c6` daba `purple` en vez de `blue`). Los hex de referencia son los acentos reales de Yaru-dark (sacados del gresource de cada tema); si el acento es casi gris (saturación < 0.10) cae a `sage`. La variante elegida se aplica a tres capas: (1) `gsettings org.gnome.desktop.interface` `gtk-theme`/`icon-theme`/`color-scheme`/`font-name` — para apps que hablan con dconf; (2) `~/.config/gtk-{3,4}.0/settings.ini` — para apps GTK sobre wlroots sin daemon de XSettings; (3) `~/.config/gtk-4.0/gtk.css` → symlink al `gtk.css` del tema Yaru, porque **libadwaita (Nautilus, Evince, …) ignora `gtk-theme` y solo carga ese archivo**. Reemplazó al viejo `chocomazapan-icon-theme-sync` (solo tocaba iconos, dejaba las ventanas con Adwaita de fábrica; ahora es un wrapper de compatibilidad) y al `gtk.css` que se generaba del wallpaper (plantilla `gtk.css.template` **eliminada** — Yaru manda ahora). Requiere los paquetes AUR `yaru-gtk-theme` + `yaru-icon-theme` (+ `ttf-ubuntu-font-family`), agregados a `install.sh`. **Ninguna app libadwaita recarga el tema en caliente** (verificado con capturas: cambiar el symlink + `gtk-theme` + togglear `color-scheme` con Nautilus abierto no hace nada). `sync_desktop()` **no** fuerza el reinicio de Nautilus a propósito — cerrarle la ventana al usuario en cada cambio de wallpaper molesta más de lo que ayuda; una ventana abierta se queda con el acento viejo hasta que se cierra y reabre.

**VS Code / VSCodium** (`chocomazapan-vscode-sync`, corre en `sync_desktop()`): `aether` sí regenera una extensión de tema local completa en `theming/current/vscode-extension/` (label `Aether`, tipo `dark`) en cada cambio de wallpaper, pero por sí sola no se activa. El script (1) enlaza esa carpeta como extensión instalada (`~/.vscode/extensions/local.theme-aether-<version>` → symlink al repo), (2) la registra en `~/.vscode/extensions/extensions.json` — VS Code ≥1.74 ya **no escanea carpetas sueltas**, solo carga lo que está en ese manifiesto, así que el symlink solo no basta (verificado: `code --list-extensions` no la ve hasta agregar la entrada), y (3) fija `"workbench.colorTheme": "Aether"` en `~/.config/Code/User/settings.json` (merge no destructivo; tolera JSONC). El nombre del tema nunca cambia entre wallpapers, solo el JSON de colores dentro de la extensión, y VS Code observa el archivo del tema activo y lo recarga en caliente — así que una ventana abierta repinta sola tras el primer `Developer: Reload Window` (necesario una única vez, para cargar la extensión nueva). `install.sh` llama al mismo script.
...
3. **SDDM con tema `chocomazapan`** — `sddm/themes/chocomazapan/` (QML Qt6 puro), copiado a `/usr/share/sddm/themes/`. **Imita a hyprlock**: fondo aleatorio borroso, reloj `HH:MM`, fecha en es_MX, campo de contraseña tipo píldora con borde de acento — mismos colores catppuccin que `hyprlock.conf`. `sddm/conf.d/chocomazapan.conf` → `/etc/sddm.conf.d/` fija el tema; SDDM es un DM real que dibuja al instante (sin el hueco de `fbcon` que dejaba ver la consola con getty/greetd). **Sin autologin** — la contraseña se escribe aquí.
   - **Fondo**: `bin/chocomazapan-sddm-bg` (corre como `alfredo` vía `chocomazapan-sddm-bg.service`, `Before=sddm.service`) elige varios wallpapers al azar de `~/Imágenes/Wallpapers` (≥1920×1080, caché por firma como `pick-lock-background.sh`), los recorta + desenfoca + oscurece con ImageMagick y los deja en `backgrounds/current-0.jpg … current-3.jpg` (ese dir lo hace escribible `install.sh` con `chown`). El `Main.qml` elige uno **distinto por monitor** según su posición (`Screen.virtualX`).
