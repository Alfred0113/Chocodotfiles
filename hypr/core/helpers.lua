-- Shared helpers for Hyprland Lua configuration. Thin convenience layer over
-- native hl.* calls (bind descriptions, launch-or-focus shortcuts, etc).
...
local function command_from(value, description)
  ...
  if value.chocomazapan then
    return "chocomazapan-launch-" .. value.chocomazapan
  ...
  elseif value.tui then
    if value.focus then
      return "chocomazapan-launch-or-focus-tui " .. shell_quote(value.tui)
    else
      return "chocomazapan-launch-tui " .. shell_quote(value.tui)
    end
  end
...
function o.launch_webapp(url)
  return "chocomazapan-launch-webapp " .. shell_quote(url)
end

function o.launch_webapp_sole(name, url)
  return "chocomazapan-launch-or-focus-webapp " .. shell_quote(name) .. " " .. shell_quote(url)
end

function o.launch_sole(match, command)
  return "chocomazapan-launch-or-focus " .. shell_quote(match) .. " " .. shell_quote(o.launch(command))
end

function o.bind_menu(keys, description, menu, options)
  o.bind(keys, description, menu and ("chocomazapan-menu " .. menu) or "chocomazapan-menu", options)
end

function o.bind_toggle(keys, description, toggle, options)
  o.bind(keys, description, "chocomazapan-toggle-" .. toggle, options)
end
